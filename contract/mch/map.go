...
func (e *contractEngine) achieveMap(a *contractData) ([]byte, error) {
	if data, err := e.virtul.GetRawData(a.header.offset, BUCKET_HEADER_SIZE); err == nil {
		return data, nil
	} else {
...
func (e *contractEngine) achieveBucket(a *contractData) (*contractBucket, error) {
	if sData, err := e.virtul.GetRawData(a.header.offset, 4); err == nil {
		bk := contractBucket{}
		size, _ := mixed.D32func(sData)
		if data, err := e.virtul.GetRawData(a.header.offset, uint64(BUCKET_HEADER_SIZE+size*8)); err != nil {
...
func (e *contractEngine) setBucket(a *contractData, bk *contractBucket) error {
	data, _ := bk.Show()
	return e.virtul.SetRawData(a.header.offset, uint64(BUCKET_HEADER_SIZE+bk.header.size*8), data)
}
...
func (e *contractEngine) reHash(a *contractData, size uint32) error {
	...
	addr, err := e.virtul.Alloc(virtul.StoreType(a.header.offset), uint64(size*8+DEFAULT_BUCKET_LENGTH))
	if err != nil {
		return fmt.Errorf("Fate Map ReHash: %v", err)
	}
	b := &contractData{
...
func (e *contractEngine) mapRemove(a *contractData) error {
	...
	return e.virtul.Free(a.header.offset)
}
...
func (e *contractEngine) insert(a, k, v *contractData) error {
	if d, err := e.find(a, k); err != nil {
		...
		case MAP:
			if k.header.typ != bk.header.ktyp {
				return errors.New("Fate Map Insert: key type don't match")
			}
			if k.header.length != 0 {
				if k0, _, err := e.findByIndex(a, 0); err == nil {
					if r, err := e.typeCmp(k, k0); err != nil {
						return fmt.Errorf("Fate Map Insert: %v", err)
					} else if r != 0 {
						return errors.New("Fate Map Insert: key type don't match")
					}
		...
		case MAP:
			if v.header.typ != bk.header.vtyp {
				return errors.New("Fate Map Insert: key type don't match")
			}
			if v.header.length != 0 {
				if _, v0, err := e.findByIndex(a, 0); err == nil {
					if r, err := e.typeCmp(v, v0); err != nil {
						return fmt.Errorf("Fate Map Insert: %v", err)
					} else if r != 0 {
						return errors.New("Fate Map Insert: key type don't match")
					}
		...
		addr, err := e.virtul.Alloc(virtul.StoreType(a.header.offset), SLOT_SIZE)
		if err != nil {
			return fmt.Errorf("Fate Map Insert: %v", err)
		}
		ktyp := k.header.typ
		k.header.typ = bk.header.ktyp
		nk, err := e.dup(k, virtul.StoreType(a.header.offset))
		k.header.typ = ktyp
		if err != nil {
			e.virtul.Free(addr)
			return fmt.Errorf("Fate Map Insert: %v", err)
		...
		nv, err := e.dup(v, virtul.StoreType(a.header.offset))
		v.header.typ = vtyp
		if err != nil {
			e.remove(nk)
			e.virtul.Free(addr)
...
func (e *contractEngine) mapCopy(a, b *contractData) error {
	...
	vtyp := bk.header.vtyp
	if a.header.length == 0 {
		bk, err = e.achieveBucket(b)
		if err != nil {
			return fmt.Errorf("Fate Map Copy: %v", err)
		}
		rk := ktyp - bk.header.ktyp
		rv := vtyp - bk.header.vtyp
		if rk != 0 || rv != 0 {
			return fmt.Errorf("Fate Map Copy: type don't match")
		}
	} else {
		r, err := e.typeCmp(a, b)
		if err != nil {
	...
	a.header.offset = c.header.offset
	e.setData(a)
	return nil
}
